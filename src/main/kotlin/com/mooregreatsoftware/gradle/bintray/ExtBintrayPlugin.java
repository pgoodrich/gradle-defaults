...
public class ExtBintrayPlugin implements Plugin<Project> {
    ...
    private static void setArtifacts(Project project, BintrayExtension bintray) {
        if (noArtifactsDefined(bintray)) {
            // TODO auto-detect configuration if a MavenPub not defined
            val mavenPublishPlugin = project.getPlugins().findPlugin(MavenPublishPlugin.class);
            if (mavenPublishPlugin == null) {
                LOG.info("No artifacts are defined for publishing to BinTray for " + project);
                return;
            }
            ...
            if (publications.isEmpty())
                throw new IllegalStateException("MavenPublications has been declared, but none have been defined for " + project);

            val defaultPublication = publications.findByName(MavenPublishPublications.PUBLICATION_NAME);
            if (defaultPublication != null) {
                bintray.setPublications(MavenPublishPublications.PUBLICATION_NAME);
            }
            else {
                val firstPublication = publications.iterator().next();
                val firstPublicationName = firstPublication.getName();
                LOG.info("Setting the MavenPublication to send to BinTray to be \"{}\"", firstPublicationName);
                bintray.setPublications(firstPublicationName);
            }
        }
    }
